...
- [ICL API Programmers Manual](#icl-api-programmers-manual)
  ...
  - [CCD Module Commands](#ccd-module-commands)
    ...
    - [ccd\_getEMGain](#ccd_getemgain)
    - [ccd\_setEMGain](#ccd_setemgain)
...
NOTE 1: Immediately call mono_moveToPosition after calibration to update the instrument's internal record. Calling mono_moveToPosition(samePositionAsCalibratedTo) is sufficient.
NOTE 2: Calibration data will reset upon switching gratings for traditional iHR320/iHR550 units, and will reset upon initialization/power cycling for all instruments.
...
>|---|---|
>| index | Integer. Used to identify which mono to control. See [mono_list](#mono_list) command|
>| locationId | Integer. Specifies which filter wheel to get the position from. See [mono_getConfig](#mono_getconfig) for the list of supported filter wheels. <br> 1 = Filter wheel 1 (Internal) <br> 2 = Filter wheel 2 (External) |
...
>|---|---|
>| index | Integer. Used to identify which mono to control. See [mono_list](#mono_list) command |
>| locationId | Integer. Specifies which filter wheel to move. See [mono_getConfig](#mono_getconfig) for the list of supported filter wheels. <br> 1 = Filter wheel 1 (Internal) <br> 2 = Filter wheel 2 (External) |
...
### <a id="ccd_getemgain"></a>ccd_getEMGain

Gets the current EM gain value.
...
>| results | description |
>|---|---|
>| gain | Integer. Current EM gain value. |
...
### <a id="ccd_setemgain"></a>ccd_setEMGain

Sets the EMCCD gain value.

_Note:_ This command should only be used after selecting an EM ADC using the [ccd_setSpeed](#ccd_setspeed) command. If an EM ADC (speed) is not present in the speeds section of [ccd_getConfig](#ccd_getconfig), then this command should not be used.

**Command Parameters:**
>| parameter  | description   |
>|---|---|
>| gain | Integer. EM gain value between 1-1000. |
...
_\* Note:_ To use the Spectra acquisition format, set yBin = ySize when defining the ROI. See [ccd_setRoi](#ccd_setroi) <br>
_\** Note:_ The Crop and Fast Kinetics acquisition formats are not supported by every CCD.
...
>| maxAttempts* | Integer. (Optional) Maximum number of attempts before the calculation times out. Default is 500 attempts|

_*Note:_ maxAttempts will only need to be increased when the ROI xSize is relatively small
...
- List of supported channels
    ...
    - ppd
    - photon
- format: `channels: ["current", "voltage", "ppd", "pmt"]`
...
**Example command:**
Get available data for all enabled channels
```json
{
    "id": 1234,
...
**Example command:**
Get available data for specific channels, as defined by the "channels" array
```json
{
    "id": 1234,
    "command": "saq3_getAvailableData",
    "parameters":{
        "index": 0,
        "channels": ["current", "voltage", "pmt", "ppd"]
    }
}
```
